...
public interface DeviceStatusRepository extends JpaRepository<DeviceStatus, Long> {
    ...
    @Query(value = "SELECT ds.* FROM device_status ds " +
                   "INNER JOIN (SELECT device_id, MAX(timestamp) as max_timestamp FROM device_status GROUP BY device_id) AS latest " +
                   "ON ds.device_id = latest.device_id AND ds.timestamp = latest.max_timestamp",
            nativeQuery = true)
