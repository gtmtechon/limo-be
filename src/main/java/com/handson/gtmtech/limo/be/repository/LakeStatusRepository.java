...
public interface LakeStatusRepository extends JpaRepository<LakeStatus, Long> {
    ...
    List<LakeStatus> findBySensorIdAndTimestampBetweenOrderByTimestampAsc(String sensorId, OffsetDateTime start, OffsetDateTime end);

    // 각 센서별 최신 데이터 조회 (가장 최근 타임스탬프 기준)
    @Query(value = "SELECT ls.* FROM lake_status ls " +
                   "INNER JOIN (SELECT sensor_id, MAX(timestamp) as max_timestamp FROM lake_status GROUP BY sensor_id) AS latest " +
                   "ON ls.sensor_id = latest.sensor_id AND ls.timestamp = latest.max_timestamp",
            nativeQuery = true)
